...
// how much current we should have at brightest point of pattern
int ideal_max_current = 450; // 175 with built-in vref
#define MAX_PWM 250
int pwm_for_max_brightness = 80;
int jumpVal = 1;
...
void loop() {
#ifdef REPRORT_TIMING
  Serial.print("Starting loop ");
  Serial.print(tickcount);
  Serial.print(" at ");
  Serial.println(millis()/DELAYFACTOR);
#endif
  // TODO:  do we want a full PID system here?

  // first we calibrate pwm_for_max_brightness to ideal_max_current and the hardware's instantaneous response
  ...
  int t = millis()/DELAYFACTOR % 1000;
  pattern_brightness =
    t < 500 ? t/500 : (1000-t)/500;
  ...
  analogWrite(CCFL_PIN, pwm_for_max_brightness*pattern_brightness);
...
void printAnalogs() {
  ...
  if (debugMode > 0) { // print detailed battery analogRead information
    ...
    Serial.print("   pwm_for_max_brightness=");
    Serial.print(pwm_for_max_brightness);
